...
function normalizeTruckName(val) {
  if (!val) return null;
  ...
  return name
    ...
    .replace(/\s*-\s*/g, " - ") // hyphen spacing normalize
    .replace(/\s*\(\s*/g, " (") // bracket spacing
    .replace(/\s*\)\s*/g, ")")
    .trim();
}
    ...
    for (const truckId of truckIds) {
      ...
      if (
        ...
        deliveredMap.set(truckId, {
          ...
          truckName: r.Truckname || r.TruckName || "",
...
function buildBookedByTruckIdMap(bookedMap) {
  const bookedByTruckId = new Map();

  for (const booked of bookedMap.values()) {
    if (!booked.truckId) continue;

    if (!bookedByTruckId.has(booked.truckId)) {
      bookedByTruckId.set(booked.truckId, []);
    }

    bookedByTruckId.get(booked.truckId).push(booked);
  }

  // 🔹 optional: sort bookings by PU time (earliest first)
  for (const bookings of bookedByTruckId.values()) {
    bookings.sort((a, b) =>
      new Date(a.puDateTime) - new Date(b.puDateTime)
    );
  }

  return bookedByTruckId;
}




async function buildInTransitTruckMap(
  loadRecords,
  truckMap,
  trailerMap,
  bookedMap
  ...
  // 🔥 BOOKED loads → regroup by truckName
  const bookedByTruckName = new Map();

  for (const booked of bookedMap.values()) {
    if (!booked?.truckName) continue;

    const key = normalize(booked.truckName);

    if (!bookedByTruckName.has(key)) {
      bookedByTruckName.set(key, []);
    }

    bookedByTruckName.get(key).push(booked);
  }

  // 🔹 sort bookings by PU time (earliest first)
  for (const bookings of bookedByTruckName.values()) {
    bookings.sort(
      (a, b) => new Date(a.puDateTime) - new Date(b.puDateTime)
    );
  }
  ...
  for (const load of loadRecords) {
    ...
    for (let tName of truckNames) {
      ...
      const truck = truckNameMap.get(normalizedName);
      if (!truck) continue;

      // ❗ already added → skip
      if (inTransitMap.has(normalizedName)) continue;
      ...
      // 🔹 NEXT BOOKED LOAD (CORRECT)
      const nextBookings =
        bookedByTruckName.get(normalize(truck.truckName)) || [];
      ...
      inTransitMap.set(normalizedName, {
        truckName: truck.truckName,
        truckId: truck.truckId,
        companyName: truck.companyName,
        samsaraVehicleId,

        // 🔸 CURRENT IN TRANSIT LOAD
...
function getBookedTruckNameSet(bookedMap) {
  const bookedTruckSet = new Set();

  for (const booked of bookedMap.values()) {
    if (booked.truckName) {
      bookedTruckSet.add(booked.truckName.trim().toUpperCase());
    }
  }

  return bookedTruckSet;
}
function getDeliveredWithoutBookedLoad(deliveredLoadMap, bookedMap) {
  const bookedTruckSet = new Set();

  // 🔹 BOOKED truck names
  for (const booked of bookedMap.values()) {
    const tName = normalizeTruckName(booked.truckName);
    if (tName) bookedTruckSet.add(tName);
  }

  const result = new Map();

  for (const [truckId, delivered] of deliveredLoadMap.entries()) {
    const deliveredTruckName = normalizeTruckName(delivered.truckName);

    // ❌ BOOKED hai → bilkul skip
    if (
      deliveredTruckName &&
      bookedTruckSet.has(deliveredTruckName)
    ) {
      continue;
    }

    // ✅ ONLY trucks with NO booked load
    result.set(truckId, delivered);
  }
